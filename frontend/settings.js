...
async function loadSettingsProfile() {
    ...
    try {
        const res = await fetch('/auth/verify', { headers: { 'Content-Type': 'application/json', 'Authorization': 'Bearer ' + localStorage.getItem('authToken') } });
    ...
    fetch('/auth/profile', { headers: { 'Content-Type': 'application/json', 'Authorization': 'Bearer ' + localStorage.getItem('authToken') } })
        .then(r => r.json())
        .then(data => {
...
async function loadEmailSettings() {
    try {
        const res  = await fetch('/auth/email-settings', { headers: { 'Content-Type': 'application/json', 'Authorization': 'Bearer ' + localStorage.getItem('authToken') } });
        const data = await res.json();
...
async function loadAdminUsers() {
    try {
        const res  = await fetch('/auth/users', { headers: { 'Content-Type': 'application/json', 'Authorization': 'Bearer ' + localStorage.getItem('authToken') } });
        const data = await res.json();
        if (!res.ok) return;
